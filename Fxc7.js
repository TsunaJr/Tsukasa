...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Kurogane\n'
            + 'ORG:Owner T.ilyasa;\n'
            + 'TEL;type=CELL;type=VOICE;waid=628311800241:+62 878-8100-8758\n'
            + 'END:VCARD'
	...
	frhan.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Sayonara @${num.split('@')[0]} TSUKASA MISS YOU:D`
				let buff = await getBuffer(ppimg)
				frhan.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
...
if (isGroup) {
		...
		}).catch(() => frhan.sendMessage(from, `Untung tsukasa bukan admin, kalo admin udah tsukasa kick!`, text , {quoted : mek}))
		} else {
		...
		break 
				default:
					...
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[tsukasa]','aqua'), 'Command Tidak Terdaftar', color(sender.split('@')[0]))
					}
			}
		} catch (e) {
